...
enum Pets {
    ...
    enum DogsProducts: String, ProductsForPets {
        ...
        var products: [Product] {
            ...
            case .chappi:
                array = [
                    Product(name: "Chappi корм для собак Мясное изобилие", description: "Полноценный сухой корм для взрослых собак всех пород. Не содержит искусственных ароматизаторов и усилителей вкуса.Мясо для силы и энергии в течение дня.Овощи, травы, злаки для отличного пищеварения. Масла и жиры для блестящей шерсти и здоровой кожи.Кальций для крепких зубов и костей.Витамины для защиты здоровья. Минералы для поддержания собаки в оптимальной форме.", image: UIImage(named: "Chappi Мясное изобилие"), price: "6", isFirstweight: true, isSecondweight: true, isThirdweight: true, isFourthweight: true),
    ...
    enum RodentsProducts: String, ProductsForPets {
        ...
        var products: [Product] {
            ...
            case .littleKing:
                array = [
                    Product(name: "Little King лакомство для грызунов (корзинка зерновая)", description: "Корзинка зерновая. Подходит для всех видов грызунов, т.к не содержит сладких ингридиентов.Благодаря твердой структуре позволяет удовлетворить естественные потребности зверьков что-то грызть, при этом стачивать острые зубы. Тем самым, обеспечивает уход за ротовой полостью питомца, укрепляет зубы и развивает челюстную мускулатуру.", image: UIImage(named: "Little King лакомство для грызунов (корзинка зерновая)"), price: "38", isFirstweight: true, isSecondweight: true, isThirdweight: false, isFourthweight: false),
                ]
            case .littleOne:
                array = [
